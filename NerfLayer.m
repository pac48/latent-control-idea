classdef NerfLayer < nnet.layer.Layer & nnet.layer.Formattable & GlobalStruct
    ...
    methods(Static)
        function llayer = loadobj(layer)
            ...
            layer.h.structure.enableRotation = false;
            llayer = layer;
        end
    end
    methods
% %         function slayer = saveobj(layer)
% %         slayer  = saveobj@super(layer); 
% %         why
% %         %             layer.h.structure.detectObjects = {};
% %         %             layer.h.structure.enableRotation = false;
% %         %             llayer = layer;
% %         end
        ...
        function layer = NerfLayer(name, allT, objNames, h, w)
            ...
            global nerf
            nerfObj = nerf.nameMap(objNames{1});
            nerfObj = nerfObj{1};
            for i = 1:length(allT)
                allT{i} = nerfObj.scaleTransform(allT{i});
            end
            layer.allT = allT;
...
%             fx  = 2*layer.width/tand(0.5*fov);    
%             fy  = 2*layer.height/tand(0.5*fov);
        ...
        function [mkptsNerf, mkptsReal, points, mconf, imgNerf, depth] = render(layer, T, angle, imReal)
            ...
            if layer.h.structure.enableRotation %rand(1) > .8
%                 angle1 = 2*(rand(1)-.5)*180;
                tmp = rand(1);
                if tmp < .25 
                    angle1 = 0;
                elseif .25  < tmp && tmp < .5
                     angle1 = 90;
                elseif .25  < tmp && tmp < .75 
                     angle1 = -90;
                else
                    angle1 = 180;
                end
                ...
                angle2 = 0;
                angle3 = 0;
            elseif layer.h.structure.enableTranslation %rand(1) > .8
                xoffset = 2*(rand(1)-.5)*.1;
                yoffset = 2*(rand(1)-.5)*.1;
            end

            T = inv(T); % cam to world
%             T(1:3, 1:3) = T(1:3, 1:3)*axang2rotm([0 0 1 pi*angle/180]);
            T(1:3, 1:3) = T(1:3, 1:3)*eul2rotm([pi*angle1/180 pi*angle2/180 pi*angle3/180]);
            T(1:3, end) = T(1:3, end) + T(1:3, 1)*xoffset + T(1:3, 2)*yoffset;  
...
%             mkptsNerf = tmp(1:2,:)' + [layer.width/2 layer.height/2];
        ...
        function updateState(layer, curInd, mkptsNerf, mkptsReal, points, mconf, imgNerf, imReal, depth)
            ...
            if ~isempty(points) && size(points, 2) >=  .5*size(layer.h.structure.(layer.Name).points{curInd}, 2) %5%.8*size(layer.h.structure.(layer.Name).points{curInd}, 2) %size(layer.h.structure.(layer.Name).points{curInd}, 2)%3 %size(layer.h.structure.(layer.Name).points, 2)
...
%             end
        ...
        function Trobot_object = getObjectTransform(layer, img, depth, Tcam1_object, Trobot_cam2)
            ...
            % points in nerf coordinates should be calculated and saved during training 
            points = getObjectPointCloud(img, depth, layer.objectName, fl, fx, fy);
            % at runtime, the points should not be extracted from the point cloud, instead just 
            % use saved points. This will enable a stable coordinate system for the object  
...
%                 Trobot_object(1:3, end) = Tobject_cam1(1:3,end); 
        ...
        function [pointsOut, mkptsRealNormalizedOut, mkptsNerfOut, imgRealOut, imgNerfOut, depthOut, Trobot_objectOut, baseTOut] = predict(layer, Tin, Img, Trobot_cam2)
            ...
            if Img(1)==1 && all(Img(1,:,:,1) == 1, 'all') && all(Img(:,:,:,1)==1, 'all')
                return
            end
            ...
            if size(Trobot_cam2, 1) == 6 % convert to T
                Trobot_cam2 = reshape(Trobot_cam2, 6,1,[]);
                Trobot_cam2 = getT(Trobot_cam2(1:3,:), Trobot_cam2(4:6,:));
            else
                Trobot_cam2 = reshape(Trobot_cam2, 4, 4, []);
            end
            ...
            if ~skipNerf %&& ~isempty(inds)
                ...
                numPoints = 0;
                bestAngle = [];
%                 if layer.h.structure.enableRotation
%                     angles = [-90 0 90 180];
%                 else
%                     angles = 0;
%                 end
            ...
            if ~skipNerf
                ...
                Trobot_object = layer.getObjectTransform(tmp.imgNerf{curInd}, tmp.depth{curInd}, Tin, Trobot_cam2);
                layer.h.structure.(layer.Name).Trobot_object{curInd} = Trobot_object;
            end
            ...
            [fl, fx, fy] = layer.getFValues();
            mkptsRealNormalized = .8*[fx -fy].*(mkptsReal - .5*offset)./offset;
            mkptsRealNormalizedOut = dlarray(mkptsRealNormalized','SSB');

 
            Trobot_objectOut = dlarray(tmp.Trobot_object{curInd}, 'SSB');
...
%                 points should be all points

                %                     Trobot_objectOut TODO: need calculate this!!!
            end
